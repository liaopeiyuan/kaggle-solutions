...
RANGE=[]
for i in range(10):
  RANGE.append(np.arange(int(139.5*1000),170*1000,10*1000))
  #RANGE.append(np.arange(int(169.5*1000),170*1000,10*1000))
"""
for i in range(7):
...
def load_image(path, mask = False):
    ...
    if mask:
        ...
        image, _ = do_center_pad_to_factor2(image,image, factor=512)
        return torch.from_numpy(image.reshape((1,512,512))).float()
...
for step in RANGE[0]:
 initial_checkpoint = '/home/liaop20/data/salt/checkpoints/list_train6_3600_ne_balanced/ResNet34_res_512'+str(step).zfill(8)+'_model.pth'
 model.load_state_dict(torch.load(initial_checkpoint, map_location=lambda storage, loc: storage))
 model.set_mode('test')
 model.eval()
 all_predictions = []
...
for i in tqdm(range(3000)):
   img = cv2.resize(pred1[i,:,:],(101,101))
   images.append(img)

del pred1

for i in tqdm(range(3000)):
   img = cv2.resize(pred2[i,:,:],(101,101))
   images.append(img)

del pred2

for i in tqdm(range(3000)):
   img = cv2.resize(pred3[i,:,:],(101,101))
   images.append(img)

del pred3

for i in tqdm(range(3000)):
   img = cv2.resize(pred4[i,:,:],(101,101))
   images.append(img)

del pred4

for i in tqdm(range(3000)):
   img = cv2.resize(pred5[i,:,:],(101,101))
   images.append(img)

del pred5

for i in tqdm(range(3000)):
   img = cv2.resize(pred6[i,:,:],(101,101))
   images.append(img)
...
submit.to_csv(RESULT+'/512.csv', index = False)
