...
RANGE=[]
for i in [1,2,6,7]:
    test_path = os.path.join('/home/liaop20/data/salt/checkpoints/ocnet_csa', 'fold'+str(i)+'_stage1')
    print(test_path)
...
for fold in FOLD:
 ...
 for i in RANGE[m]:
    ...
    #00099500_model.pth
    initial_checkpoint = '/home/liaop20/data/salt/checkpoints/ocnet_csa'+'/fold'+fold+'_stage1'+'/'+i+'.pth'
    model.load_state_dict(torch.load(initial_checkpoint, map_location=lambda storage, loc: storage))
    model.set_mode('test')
    model.eval()
    all_predictions = []
    ...
    pickle.dump( temp, open( "ocnet"+fold+i+".p", "wb" ) )
 m=m+1
