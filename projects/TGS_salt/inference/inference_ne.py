...
RANGE=[]
for i in range(10):
    RANGE.append(np.arange(int(9.5*1000),60*1000,10*1000))
...
#for fold in ["8"]:
 ...
 #for i in np.arange(200,460,50):
    ...
    if i > 59.5*1000:
        ...
        stage=""
    initial_checkpoint = CHECKPOINTS+'/list_train'+fold+'_3600_ne_balanced/'+'ResNet34_'+stage+str(i).zfill(8)+'_model.pth'
    model.load_state_dict(torch.load(initial_checkpoint, map_location=lambda storage, loc: storage))
    model.set_mode('test')
    model.eval()
    all_predictions = []
    ...
    pickle.dump( np.vstack(all_predictions)[:, 0, :, :], open( fold+str(i).zfill(8)+".p", "wb" ) )
 m=m+1
