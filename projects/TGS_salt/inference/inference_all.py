...
RANGE=[]
for i in range(1):
    RANGE.append(np.arange(int(49.5*1000),100*1000,10*1000))
...
#for fold in ["8"]:
 ...
 #for i in np.arange(200,460,50):
    ...
    if i > 59.5*1000:
        ...
        stage=""
    #00099500_model.pth
    initial_checkpoint = CHECKPOINTS+'/list_train'+fold+'_3600/'+'ResNet34_all_128'+str(i).zfill(8)+'_model.pth'
    model.load_state_dict(torch.load(initial_checkpoint, map_location=lambda storage, loc: storage))
    model.set_mode('test')
    model.eval()
    all_predictions = []
    ...
    pickle.dump( temp, open( "all_"+fold+str(i).zfill(8)+".p", "wb" ) )
 m=m+1
...
#for alist in RANGE:
#    sums=sums+len(alist)
all_predictions_stacked=all_predictions_stacked/(len(FOLD)*len(RANGE[0]))
