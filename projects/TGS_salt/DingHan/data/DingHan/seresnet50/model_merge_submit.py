...
out_dir = \
    'E:\\DHWorkStation\\Project\\tgs_pytorch\\output\\seresnext50_bn\\fold4\\'
...
#swa_ 3600_1
initial_checkpoints=[
    'E:\\DHWorkStation\\Project\\tgs_pytorch\\output\\seresnext50_bn\\fold4\\checkpoint\\00070200_loss_0.231_model.pth',
    ...
    'E:\\DHWorkStation\\Project\\tgs_pytorch\\output\\seresnext50_bn\\fold4\\checkpoint\\00056000_loss_0.230_model.pth',
...
def merge_prob(output_files,augment):
    ...
    all_prob=(all_prob * 255).astype(np.uint8)
    np.save(out_dir + '/test/merged_submit-%s-%s.prob.uint8.npy' % (split, augment), all_prob)
    #return all_prob


def run_submit(augment,thres,prob_preds=None):
    print('running submit')
    if augment in ['null', 'flip']:
        augmentation = [
            1, out_dir + '/test/merged_submit-%s-%s.prob.uint8.npy' % (split, augment),
        ]
        csv_file = out_dir + '/test/merged_submit-%s-%s.csv' % (split, augment)

    if augment == 'aug2':
        augmentation = [
            1, out_dir + '/test/merged_submit-%s-%s.prob.uint8.npy' % (split, 'null'),
            1, out_dir + '/test/merged_submit-%s-%s.prob.uint8.npy' % (split, 'flip'),
        ]
        csv_file = out_dir + '/test/merged_submit-%s-%s.csv' % (split, augment)
...
if mode == 'valid':
    for a in ['null', 'flip']:
        ...
        for ckpt in initial_checkpoints:
            print('a=', a)
            print(ckpt)
            print(ckpt.split('\\')[-1].split('.')[0])
            if os.path.exists(out_dir+('/test/%s-%s-%s.prob.uint8.npy' % (ckpt.split('\\')[-1].split('.')[0],split, a))):
                print('exist: ',out_dir+('/test/%s-%s-%s.prob.uint8.npy' % (ckpt.split('\\')[-1].split('.')[0],split, a)))
            else:
                run_predict(a,ckpt,ckpt.split('\\')[-1].split('.')[0])
            merge_files.append(out_dir+('/test/%s-%s-%s.prob.uint8.npy' % (ckpt.split('\\')[-1].split('.')[0],split, a)))
        merge_prob(merge_files,a)

        # run_submit('aug2')

    for t in np.arange(0.35,0.6,0.005):
        print(t)
        for a in [ 'aug2']:
            print('a=', a)
            run_submit(a,t)
