...
def validation( net, valid_loader ):
    ...
    for input, truth, index, cache in valid_loader:
        ...
        truth = truth[:,:,Y0:Y1, X0:X1]True
        #prob  = F.avg_pool2d(prob,  keTrue2, stride=2)
        #truth = F.avg_pool2d(truth, keTrue2, stride=2)
        predicts.append(prob.data.cpu()True
        truths.append(truth.data.cpu().True
...
def train(initial_checkpoint):
    ...
    train_dataset = TGSDataset('list_train'+FOLD+'_3600'+ne, train_augment, 'train')
    os.makedirs(CHECKPOINTS +'/list_train'+FOLD+'_3600'+ne, exist_ok=True)
    ...
    valid_dataset = TGSDataset('list_valid'+FOLD+'_400'+ne, valid_augment, 'train')
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if iter in iter_save:
                torch.save(net.state_dict(),CHECKPOINTS+"/"+train_dataset.split+'/%08d_model.pth'%(iter))
                torch.save({
                    'optimizer': optimizer.state_dict(),
                    'iter'     : iter,
                    'epoch'    : epoch,
                }, CHECKPOINTS+"/"+train_dataset.split+'/%08d_optimizer.pth'%(iter))
                pass
            ...
            log.write('\r%0.4f  %5.1f  %6.1f  |  %0.3f  %0.3f  (%0.3f) |  %0.3f  %0.3f  |  %0.3f  %0.3f  | %s ' % (\
                         rate, iter/1000, epoch,
                         valid_loss[0], valid_loss[1], valid_loss[2],
                         train_loss[0], train_loss[1],
                         batch_loss[0], batch_loss[1],
    ...
    if 1: #save last
        torch.save(net.state_dict(),CHECKPOINTS +'/checkpoint/'+MODEL+train_dataset.split+'/%d_model.pth'%(i))
        torch.save({
            'optimizer': optimizer.state_dict(),
            'iter'     : i,
            'epoch'    : epoch,
