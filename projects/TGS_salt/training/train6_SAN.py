...
from models.TGS_salt.VGG16Modified import VGG16Modified as SAN

SIZE = 101
FACTOR = 128
FOLD = 6
ne = "_ne"
initial_checkpoint ='/data/ml-arsenal/projects/TGS_salt/ResNet34_OHEM00149500_model.pth'#None
MODEL = "ResNet34_"
OHEM = "SAN"
PAD  = 27
...
def validation( net,guidance_module, valid_loader ):
    ...
    for input, truth, index, cache in valid_loader:
        ...
        with torch.no_grad():
            ...
            refined_map = guidance_module(input,prob)
            prob = F.sigmoid(refined_map)
            #criterion = CrossEntropyLoss2d(size_average=False, ignore_index=-1).cuda()
            #loss = criterion(refined_map, truth.type("torch.cuda.LongTensor").resize(truth.size()[0],truth.size()[2],truth.size()[3])) / input.size(0)
            metric = torch.nn.BCEWithLogitsLoss(size_average=True, reduction='none')
            loss= net.focal_loss(refined_map, truth, 1.0, 0.5, 0.25) +metric(refined_map.squeeze(1),truth.squeeze(1))
            #loss  = net.criterion(logit, truth)
            dice  = net.metric(refined_map, truth)
            #print(loss)
            #dice = 0
        batch_size = len(index)
        valid_loss += batch_size*np.array(( loss.item(), 0, 0))
...
def train(initial_checkpoint):
    ...
    train_dataset = TGSDataset('list_train'+str(FOLD)+'_3600'+ne+"_balanced", train_augment, 'train')
    os.makedirs(CHECKPOINTS +'/list_train'+str(FOLD)+'_3600'+ne+"_balanced", exist_ok=True)
    ...
    valid_dataset = TGSDataset('list_valid'+str(FOLD)+'_400'+ne+"_balanced", valid_augment, 'train')
    ...
    net = Net().cuda()
    guidance_module = SAN(n_classes=1).cuda()
    ...
    scheduler = None#lambda x: (0.01/2)*(np.cos(PI*(np.mod(x-1,300*1000/30)/(300*1000/30)))+1)
    #log.write(scheduler(1))
    #log.write(scheduler(5000))
    #log.write(scheduler(10001))

    optimizer = optim.SGD(filter(lambda p: p.requires_grad, guidance_module.parameters()),
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if iter % iter_valid==0:
                net.set_mode('valid')
                valid_loss = validation(net,guidance_module, valid_loader)
                net.set_mode('train')
            ...
            if iter in iter_save:
                torch.save(guidance_module.state_dict(),CHECKPOINTS+"/"+train_dataset.split+'/'+MODEL+OHEM+'%08d_model.pth'%(iter))
                torch.save({
            ...
            prob = F.sigmoid(refined_map) 
            #criterion = CrossEntropyLoss2d(size_average=False, ignore_index=-1).cuda()
            #loss = criterion(refined_map, target) / input.size(0)
            metric = torch.nn.BCEWithLogitsLoss(size_average=True, reduction='none')
            loss= net.focal_loss(refined_map, truth, 1.0, 0.5, 0.25) + metric(refined_map.squeeze(1),truth.squeeze(1))
            """
            if OHEM == "OHEM":
