...
FOLD = 6
ne = "_ne"
initial_checkpoint = None#'/home/liaop20/data/salt/checkpoints/list_train6_3600_ne_balanced/ResNet34_25600151000_model.pth'#None
MODEL = "ResNet34_"
OHEM = "relu"
PAD  = 27
...
def validation( net, valid_loader ):
    ...
    for input, truth, index, cache in valid_loader:
        ...
        with torch.no_grad():
            ...
            loss  = net.criterion3(logit, truth)
...
def train(initial_checkpoint):
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if OHEM == "OHEM":
                loss = net.focal_loss(logit, truth, 1.0, 0.5, 0.25) + net.criterion3(logit, truth)
            else:
                loss  = net.criterion3(logit, truth)
