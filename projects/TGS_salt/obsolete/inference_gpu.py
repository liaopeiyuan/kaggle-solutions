...
initial_checkpoint = CHECKPOINTS+'/list_train8_3600/00070000_model.pth'
...
model.eval()
all_predictions = []
for image in tqdm(data.DataLoader(test_dataset, batch_size = 50)):
...
file_list_train, file_list_val= train_test_split(
    file_list,
    test_size=0.4, random_state=1200)
dataset_val = InferenceDataset(train_path, file_list_val)
...
metric_by_threshold = []
for threshold in np.linspace(0.4, 0.5, 50):
