...
from models.TGS_salt.Unet_scSE_hyper import Unet_scSE_hyper as Net
...
def train():
    ...
    initial_checkpoint = '/home/liaop20/ml-arsenal/projects/TGS_salt/checkpoint/list_train8_3600/00065000_model.pth'
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if iter in iter_save:
                torch.save(net.state_dict(),RESULT +'/checkpoint/'+train_dataset.split+'/%08d_model.pth'%(iter))
                torch.save({
                    'optimizer': optimizer.state_dict(),
                    'iter'     : iter,
                    'epoch'    : epoch,
                }, RESULT +'/checkpoint/'+train_dataset.split+'/%08d_optimizer.pth'%(iter))
                pass
    ...
    if 1: #save last
        torch.save(net.state_dict(),RESULT +'/checkpoint/'+train_dataset.split+'%d_model.pth'%(i))
        torch.save({
            'optimizer': optimizer.state_dict(),
            'iter'     : i,
            'epoch'    : epoch,
        }, RESULT +'/checkpoint/'+train_dataset.split+'%d_optimizer.pth'%(i))
