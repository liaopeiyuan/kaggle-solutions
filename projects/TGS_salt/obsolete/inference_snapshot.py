...
FOLD="8"
RANGE=np.arange(40000,76000,5000)
batch_size=30
...
valid_dataset = TGSDataset('list_valid'+FOLD+'_400', valid_augment, 'train')
...
for i in RANGE:
    ...
    all_predictions_stacked.append(np.vstack(all_predictions)[:, 0, :, :])
all_predictions_stacked=np.array(all_predictions_stacked)
