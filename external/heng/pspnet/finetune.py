...
def run_train():
    ...
    out_dir = '.'
    initial_checkpoint = 'checkpoint/00095000_model.pth'\
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if iter in iter_save:
                torch.save(net.state_dict(),out_dir +'/checkpoint/%08d_model_f.pth'%(iter))
                torch.save({
                    'optimizer': optimizer.state_dict(),
                    'iter'     : iter,
                    'epoch'    : epoch,
                }, out_dir +'/checkpoint/%08d_optimizer_f.pth'%(iter))
                pass
