...
def do_valid( net, valid_loader ):
    ...
    for input, truth, index, cache in valid_loader:
        ...
        truth = truth.cuda()
        with torch.no_grad():
            logit,classes = data_parallel(net,input) #net(input)
...
def run_train():
    ...
    train_dataset = TsgDataset('list_train8_3600', train_augment, 'train')
    ...
    valid_dataset = TsgDataset('list_valid8_400', valid_augment, 'train')
    ...
    net, starting_epoch = build_network(None, 'resnet34')
    ...
    while  iter<num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            #seg_criterion = L.lovasz_hinge(weight=class_weights)
            #cls_criterion = nn.BCEWithLogitsLoss()
            logit, out_cls = data_parallel(net,input)
            #seg_loss, cls_loss = net.criterion(out, truth), cls_criterion(out_cls, y_cls)
